...
- **Title/end cards** are config-driven: `titleText/titleSub/titleFont/titleSize/titleColor/titleSubColor/titleY/subY/bunnyX/bunnyY/endTagline` (rail → Title & end cards). On the TITLE/ENDING screens in EDIT mode the title, subtitle and bunny are **card elements** (`r.cardEls`): click one and the inspector shows its text/font/size/color/position, drag moves it (text stays centered — only Y). Entering a card screen in EDIT auto-opens the inspector with a hint. SHARE LINK / COPY CONFIG carry everything.
