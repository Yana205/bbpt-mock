...
- **Tea party proportions**: the friends' art is on a 64px grid, so `CFG.teaScale` (default **0.5**, rail → Characters & sprites) zooms the whole composition — deco, hotspots, spawn — about a floor pivot at build time. In floors 1–3 the player is the same `charRef` **remapped to the purple ramp** (`CFG.sprChar`, default on): the pink facade drains when she leaves the party.
