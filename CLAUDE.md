...
- `mode: 'designer'` = **EDIT**: everything visible. Backtick toggles the rail independently. **Layout editor v2** (2026-08-29): click **selects** (dashed outline + red handles + the object's interact radius; a floating INSPECTOR window shows X/Y/size/reach/label/scale/flip/z-order/dup/del), drag moves, Shift+arrows nudge 1px, Del removes scenery, Ctrl/Cmd+Z undoes, SNAP = 4px grid, HITBOXES overlays every interact radius + the walkable rect, **T** flips to playtest-and-back to feel the room. The edit bar is deliberately minimal (2026-08-29, Yana's call): only ▣ INSPECTOR + ♫ MUSIC — snap/hitboxes/undo stay keyboard-only (Ctrl/Cmd+Z), deeper edits go through Claude Code. The selection's **bottom-right grip drag-resizes** (items/doors: hotspot w/h · scenery: scale). Interact radii derive from each object's own x/y (+ optional per-object `reach`), so they always travel with it; tea-party hotspots carry `follow:'<decoId>'` and move with their art. Positions persist per room in `localStorage['bb.layout2']` (deco keyed by stable `_id`, plus add/del/zorder/scale); COPY LAYOUT exports it all (incl. room music) to bake in, RESET LAYOUT forgets. Rooms declare scenery in `r.deco`; `bigChar:true` (tea party) draws the player as the animated `charWalk`/`charRef` sprite.
- **Tea party room** (2026-08-29): baked **1:1 from the friends' Figma frame** (figma.com/design/lduq59Tib9grkhvqDIZmXw "BB" / Frame 1, natively 320×180) — verified pixel-identical to the frame export. New assets: `teaWall` (wallpaper band, drawn OVER the window tops), `tableLong`, `vivi`, `mara`, `bunnySmall` (44×64 plush pre-scaled NN to its placed 21×31), `charRef` (hero holding the bunny) + `charWalk` (3-frame idle spritesheet from `hero_animation.gif`, 100ms/frame). Figma sources in `art/friends/figma-room/`. Gotcha: Figma metadata reports flipped layers at their transform origin, not visual bounds (visual x = x − width). The room renders **strictly 1:1** — `teaScale` is retired (stale saved values are ignored, pre-Figma tea-party layouts auto-purge on load). In floors 1–3 the player is the same `charRef` **remapped to the purple ramp** (`CFG.sprChar`, default on): the pink facade drains when she leaves the party — and she carries the bunny. **Dialogs render in-canvas game-wide** (`drawDialog`, 2026-08-29 — Figma Page 1 `text-boxes-teaparty` frames): the friends' `teaBox` (288×47 @ 16,125) + portrait sprites (`portraitVivi/Mara/Bunny/Hero` = Figma enemy1/enemy2/bunnydoll/char_ref portraits, exported as node screenshots since they're cropped fills); the tell mirrors the portrait + red wash. The DOM dialog is removed. Legacy procedural girls (bigGirl/miniGirl) and bow/table/chairBack assets are removed.
