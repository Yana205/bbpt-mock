...
- **Title card = baked art** (2026-08-29): the friends' `bunny-boo-start-screen.png` (768×432, archived in `art/friends/`), Lanczos-downscaled to `art/runtime/titleCard.png` (320×180) and drawn full-screen; only the blinking PRESS SPACE (dark-rose `#a54a61` @ y152) is code. The old title config knobs/card elements are removed. The **end card** stays config-driven: `titleFont/titleColor/endTagline` (rail → End card), with `endTagline` still a clickable card element in EDIT. SHARE LINK / COPY CONFIG carry everything.
